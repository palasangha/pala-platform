...
export const ocrAPI = {
  ...
  openInTagGUI: async (imageId: string): Promise<{ work_dir: string; txt_file: string; status: string }> => {
    const { data } = await api.post(`/ocr/image/${imageId}/taggui/open`);
    return data;
  },

  syncFromTagGUI: async (imageId: string): Promise<{ text: string }> => {
    const { data } = await api.get(`/ocr/image/${imageId}/taggui/sync`);
    return data;
  },
};
