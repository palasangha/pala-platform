...
export const OCRReview: React.FC = () => {
  ...
  // TagGUI state
  const [tagguiOpened, setTagguiOpened] = useState(false);
  const [tagguiWorkDir, setTagguiWorkDir] = useState('');
  const [tagguiSyncing, setTagguiSyncing] = useState(false);
  ...
  const handleOpenTagGUI = async () => {
    if (!imageId) return;
    try {
      const result = await ocrAPI.openInTagGUI(imageId);
      setTagguiOpened(true);
      setTagguiWorkDir(result.work_dir);
    } catch (err) {
      setError('Failed to launch TagGUI');
    }
  };

  const handleSyncTagGUI = async () => {
    if (!imageId) return;
    setTagguiSyncing(true);
    try {
      const result = await ocrAPI.syncFromTagGUI(imageId);
      setOcrText(result.text);
      setHasChanges(true);
    } catch (err) {
      setError('Failed to sync from TagGUI — ensure annotation is saved in TagGUI first');
    } finally {
      setTagguiSyncing(false);
    }
  };
            ...
            <>
              <div className="p-4 border-b">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={handleOpenTagGUI}
                      className="flex items-center px-3 py-1.5 bg-purple-600 text-white text-sm rounded-md hover:bg-purple-700"
                      title="Open image in TagGUI for annotation"
                    >
                      Open in TagGUI
                    </button>
                    {tagguiOpened && (
                      <button
                        onClick={handleSyncTagGUI}
                        disabled={tagguiSyncing}
                        className="flex items-center px-3 py-1.5 bg-orange-500 text-white text-sm rounded-md hover:bg-orange-600 disabled:opacity-50"
                        title={`Sync annotation from: ${tagguiWorkDir}`}
                      >
                        {tagguiSyncing ? 'Syncing...' : 'Sync from TagGUI'}
                      </button>
                    )}
                  </div>
                </div>
