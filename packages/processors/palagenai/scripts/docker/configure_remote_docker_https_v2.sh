...
if ! grep -q "registry.palagenai.com" /etc/hosts 2>/dev/null; then
    echo "$SUDO_PASSWORD" | sudo -S sh -c "echo '$REGISTRY_SERVER registry.palagenai.com' >> /etc/hosts"
    echo "✓ Added registry.palagenai.com to /etc/hosts"
else
    # Update existing entry
    echo "$SUDO_PASSWORD" | sudo -S sed -i '/registry.palagenai.com/d' /etc/hosts
    echo "$SUDO_PASSWORD" | sudo -S sh -c "echo '$REGISTRY_SERVER registry.palagenai.com' >> /etc/hosts"
...
if curl -sk https://registry.palagenai.com:7103/v2/ > /dev/null 2>&1; then
    ...
    curl -sk https://registry.palagenai.com:7103/v2/_catalog
...
if [ $? -eq 0 ]; then
    ...
    DOCKER_API_VERSION=1.41 docker -H tcp://$REMOTE_HOST:2375 pull registry.palagenai.com:7103/gvpocr-worker-updated:latest
