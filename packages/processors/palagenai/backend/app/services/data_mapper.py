...
class DataMapper:
    ...
    def merge_enriched_with_ocr(
        ...
        try:
            ...
            # Generate AMI filename from merged enriched data
            try:
                from .ami_filename_generator import AMIFilenameGenerator
                gen = AMIFilenameGenerator()
                fname_result = gen.generate(base_data, ocr_data.get('file_info', {}).get('filename', ''))
                base_data['generated_filename'] = fname_result['generated_filename']
                base_data['generated_filename_components'] = fname_result['components']
            except Exception as e:
                logger.warning(f"AMI filename generation skipped: {e}")
