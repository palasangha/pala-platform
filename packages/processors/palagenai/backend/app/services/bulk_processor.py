                ...
                if self.enable_enrichment:
                    ...
                    try:
                        ...
                        if enriched_data:
                            ...
                            # Generate AMI-standard filename from enriched metadata
                            try:
                                from .ami_filename_generator import AMIFilenameGenerator
                                gen = AMIFilenameGenerator()
                                filename_result = gen.generate(enriched_data, filename)
                                result['generated_filename'] = filename_result['generated_filename']
                                result['generated_filename_components'] = filename_result['components']
                            except Exception as fname_err:
                                logger.warning(f"Filename generation failed for {filename}: {fname_err}")
                        else:
