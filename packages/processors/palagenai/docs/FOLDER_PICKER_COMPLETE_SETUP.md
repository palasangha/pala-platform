...
### Problem
Browser showed: `GET https://palagenai.com/api/bulk/history?page=1&limit=10 401 (Unauthorized)`

### Root Cause
Frontend `.env` was configured with relative path `/api`, which resolved to production domain `palagenai.com` instead of local backend.
